...
> **工作語言是繁體中文。** `SKILL.md` 與報告都是中文，而且區段名稱不是風格選擇:`evals/format_check.py` 逐字比對它們(`"survivors", "二、存活候選"`),換成英文標題的報告過不了自己的查核器。這一點英文版 README 也寫了，寫在他還沒裝之前看得到的地方。
>
> **但文獻檢索一律用英文**——用非英文查一個領域的正規術語，是把「還沒有人做過」誤判出來最快的方式。那條規則寫在 `SKILL.md` 裡，不隨報告語言改變。
...
守住這一頁的就是 `doc_scan.py`：規則條數它從 `format_check.py` 讀、樣本數從磁碟數、突變體數從 `mutation_test.py` 讀，只要 README 裡寫死的數字、檔案路徑、指令或相對連結跟倉庫對不上就回非零。下面每一個**總數**都在它的守備範圍內——寫錯會讓一次執行變紅，而不是安靜地留在文字裡。它刻意不讀的是*部分數*——「其中三筆」、「剩下六筆」——因為數一個集合的一部分不是對那個集合的宣稱，把它們當成總數去讀，會對正確的句子開槍。那些數字沒有人守，而這一段以前是這樣說的：它宣稱每一個數字都被守著，同時兩百行以下有一個寫成中文字的總數在中英兩頁之間飄掉了，而執行仍然 exit 0。現在那個總數兩種語言都讀、阿拉伯數字和中文字都讀；部分數還是只能靠你自己。
...
- **10 筆是刻意不動。** 其中三筆長在地形報告的表格上——在表格中間插一行 `## ` 標題，後面每一列都會掉出那張表；一列同時掉了行首直線又用全形 `｜`，既不進表也不會被回報；〈家族數〉寫成「三」，它的對帳一聲不吭地跳過。這次重構刻意沒有動地形：那套規則本來就**刻意很薄**，為了搆到它而重寫整支表格解析器，比例不對。第四筆是 `report-start`／`report-end` 標記，它仍然挖得掉一行區塊錨點指不到的散文。剩下六筆是「新的 containment 問題刻意只問到一半」的代價——把其中任何一個收緊，換來的不是假綠燈變綠燈，而是某種情況下的**靜默丟棄**，或是對一份合規報告開出假紅燈，而那正是這個 repo 白紙黑字寫下「不要那樣換」的方向。每一列都帶著自己的論證，也沒有人打算回頭。
