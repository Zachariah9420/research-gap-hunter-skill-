...
NUM_WORDS = {
    ...
    # to_int 會回 None,claim_check 就當「文件沒宣稱」而放行——那是靜默漏掉,
    # 不是通過。這一段補齊過一次真實的漏檢(英文寫 eleven、中文寫十二,兩邊都沒被讀到)。
...
if score_rows:
    ...
    # 未關列數在兩份 README 各被說兩次:一次是總數(上面那條讀的),一次是
    # 帶出拆解的那一句。第二次曾經飄掉而沒有被抓到——英文寫 eleven、中文寫
    # 十二,兩邊不一致,而當時所有 pattern 都只認阿拉伯數字,所以掃描器 exit 0。
    # 同一個數字說兩次就要被讀兩次,否則第二次等於沒有守衛。
    claim_check(
        "仍打得到的列數(帶出拆解的那一句)", n_open,
        [re.compile(r"[Oo]f[ \t]+the[ \t]+([A-Za-z]+|\d+)[ \t]+still[ \t]+open"),
         re.compile(r"仍然打得到的那\s*([一二三四五六七八九十]+|\d+)\s*筆")],
        READMES,
    )
