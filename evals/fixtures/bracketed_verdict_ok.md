...
| C05 以穿戴裝置的 GPS 軌跡切出居民在公園內的活動片段 | 〔待驗證〕**不列入存活** | 領域正規術語未定：`park based physical activity GPS` 與 `greenspace exposure trajectory segmentation` 回到兩個不相通的社群 | 先讀兩個社群各一篇綜述確定術語，再用該術語重跑兩輪檢索，才回來判定 |
