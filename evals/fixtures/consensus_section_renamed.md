...
> 相對於 good_report.md，本檔刻意壞掉一處：〈一、領域共識與未被質疑的預設〉被改名。以前這一行會讓第一節的預設 3→0、unreadable 0、離開碼 0——而 C01 是 G3 候選，於是報告還會多出一句「G3 候選指到第一節沒有的預設 A1」，把作者送去補一條就躺在那裡的預設。預設行現在整份文件掃，只剩標題本身要修。
