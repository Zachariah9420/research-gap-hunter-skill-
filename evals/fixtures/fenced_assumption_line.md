...
> 相對於 good_report.md，本檔刻意壞掉一處：A3 那一行被包進一個 ```text 圍欄裡。字沒動、位置也還在第一節，所以每一個錨點照樣對得上——壞掉的是**那一行變成了程式碼**：讀者看到的是一塊 code block，不是報告內容。圍欄是「兩邊都不算」的區域（查核器不保證讀它、讀者不當它是內容），而兩邊都不算的區域什麼都裝得下。
