...
> 相對於 good_landscape.md，本檔刻意壞掉一處：〈一、一眼表〉整節不見了。這一份釘的是文件層的存在性——不問那張表落在哪一節、那一節叫什麼名字。會需要它，是因為「表被貼成純文字」與「節標題被改寫」同時發生時，形狀與節名兩個定位管道會一起失效；而一眼表是這份報告裡唯一沒有交叉對帳的結構，沒有任何規則會數它的列數，於是整張表消失而報告全綠。
