...
> 相對於 good_landscape.md，本檔刻意壞掉一處：報告最後多了一個 `json rgh-block` 區塊（把缺口報告的樣板整段貼過來最容易產生這一種）。SKILL.md〈rgh-block〉寫著地形報告不寫區塊，所以**沒有任何規則會驗它**——LandscapeChecker 沒有 check_block，也不該有。以前這裡是三件事一起發生：不被驗證、被剝出散文、於是連 LANG-01 與 LVOCAB-01 都掃不到它，一個地形報告可以在圍欄裡寫下缺口獵捕才有的判定詞彙而全綠。剝除現在只發生在缺口報告，而區塊本身在這裡是一筆違規。

## 一、一眼表

| 家族 | 一句話 | 買到什麼 | 付出什麼 | 狀態 | 錨定文獻數 |
|---|---|---|---|---|---|
| F1 遙測綠覆指數 | 用衛星影像算出每個地址周邊有多少綠 | 全域可比、成本近零 | 分不出可及與可看 | 飽和 | 4 |
| F2 圖資緩衝區與設施清冊 | 用公園多邊形與路網算可及性 | 對得上政策單位的語言 | 圖資更新落後現地 | 飽和 | 4 |
| F3 街景影像評分 | 從街道視角的畫面評綠意品質 | 拿到人眼高度的視覺經驗 | 評分判準難跨城市搬 | 活躍 | 5 |
| F4 使用者自陳問卷 | 直接問居民去了幾次、覺得多綠 | 拿得到動機與感受 | 回憶偏誤與社會期許 | 飽和 | 4 |
| F5 行動裝置定位軌跡 | 用手機軌跡切出停留在綠地的片段 | 個人層次的實際停留 | 樣本自選、隱私門檻高 | 活躍 | 4 |
| F6 定點人流計數 | 在出入口用計數器數通過人次 | 連續、不靠受訪者記憶 | 只有總量、沒有身分 | 新興 | 3 |
| F7 現地稽核量表 | 訓練過的稽核員到現場逐項評分 | 拿到設施層級的細節 | 還沒查到 | 〔涵蓋不足〕 | 2 |
...
- **狀態**：飽和｜`satellite vegetation index greenness exposure` 在 Semantic Scholar 的寬鬆關鍵字總數 318 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 3 筆
...
- **狀態**：飽和｜`park accessibility buffer network analysis` 在 Semantic Scholar 的寬鬆關鍵字總數 264 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 2 筆
...
- **狀態**：活躍｜`street view imagery eye-level greenness` 在 Semantic Scholar 的寬鬆關鍵字總數 187 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 13 筆
...
- **狀態**：飽和｜`self-reported park visitation questionnaire validity` 在 Semantic Scholar 的寬鬆關鍵字總數 221 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 2 筆
...
- `street view imagery eye-level greenness` 在 Semantic Scholar 的寬鬆關鍵字總數 187 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 13 筆——讀到的那一頁有超過一半落在近三年。
- `gps trajectory green space exposure dwell time` 在 Semantic Scholar 的寬鬆關鍵字總數 143 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 12 筆。
- `pedestrian counter park footfall continuous` 在 Semantic Scholar 的寬鬆關鍵字總數 38 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 18 筆；總數是本次盤點裡最小的，而讀到的那一頁近三年占比最高。
- `satellite vegetation index greenness exposure` 在 Semantic Scholar 的寬鬆關鍵字總數 318 筆（工具自報，未加年份條件）；本次實際讀取回傳的前 20 筆，其中 2023 之後 3 筆；總數最大，而讀到的那一頁近三年占比最低，回顧文獻多半把它當成基準線而不是研究對象。
...
|---|---|---|---|---|
| 1 | （家族清單） | `green space exposure assessment review` | 總數 46／讀 20 | A review of greenness exposure assessment methods / Measuring exposure to urban nature: a methodological review / Green space and health: exposure metrics revisited |
| 2 | （家族清單） | `green space exposure measurement survey` | 總數 31／讀 20 | Surveying measurement approaches for urban green exposure / From satellites to surveys: a methodological survey / Exposure science for urban nature |
| 3 | （家族清單） | `green space exposure taxonomy` | 總數 12／讀 12 | A taxonomy of green space exposure metrics / Classifying environmental exposure measures / Toward a shared vocabulary for greenness exposure |
| 4 | F1 | `satellite vegetation index greenness exposure` | 總數 318／讀 20 | Satellite-derived vegetation indices as exposure metrics in health research / Buffer size sensitivity in residential greenness studies / Seasonal variation in vegetation index exposure estimates |
| 5 | F2 | `park accessibility buffer network analysis` | 總數 264／讀 20 | Accessibility measures of urban parks: a comparison / Network distance versus Euclidean buffers in park accessibility / Service area analysis for neighbourhood open space |
| 6 | F3 | `street view imagery eye-level greenness` | 總數 187／讀 20 | Street view imagery for eye-level greenness assessment / Human versus automated scoring of streetscape quality / Eye-level and overhead greenness give different exposure estimates |
| 7 | F4 | `self-reported park visitation questionnaire validity` | 總數 221／讀 20 | Self-reported park visitation: validity against objective measures / Recall bias in leisure activity questionnaires / Questionnaire design effects on reported outdoor time |
| 8 | F5 | `gps trajectory green space exposure dwell time` | 總數 143／讀 20 | GPS-based measurement of green space exposure / Dwell-time segmentation for park visit detection / Positional error in urban canyons and activity space delineation |
| 9 | F6 | `pedestrian counter park footfall continuous` | 總數 38／讀 20 | Automated pedestrian counters in park evaluation / Continuous footfall data for park renovation assessment / Undercounting in infrared trail counters |
| 10 | W2 | `measurement invariance environmental exposure subgroup` | 總數 57／讀 20 | Differential misclassification of greenness exposure across age groups / Measurement invariance in environmental audit instruments / Subgroup differences in exposure measurement error |
...
```json rgh-block
{"schema": "rgh-block/1", "settlement": {"generated": 0, "survived": 0, "pending": 0, "killed": 0}, "assumptions": []}
```
