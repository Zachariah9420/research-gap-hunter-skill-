...
- **缺口類型**：G3 預設反轉（反轉 A1）
- **新穎性判定**：ADJACENT
- **佐證資料**：查詢 1 `green space exposure measurement physical activity`（回傳 12 筆）；查詢 2 `park visitation frequency accelerometer physical activity`（回傳 9 筆）；查詢 3 `residential greenness buffer versus park use exposure`（回傳 7 筆）
