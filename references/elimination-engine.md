...
- **`retract`**：回 🚨 → 淘汰無效；回 ❓ 查詢失敗**不等於乾淨**，報告寫「未查」。`coverage_caveat` 在**每一筆乾淨記錄**上都會出現（與年份無關，不要拿它當年份判斷的依據）；**殺手文獻的年份要從 `search`／`verify` 的書目取，`retract` 的回傳沒有年份欄**。殺手文獻發表於 2010 年前時，把 `coverage_caveat` 原文抄進報告。
